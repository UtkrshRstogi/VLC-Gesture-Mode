...
# external camera.
hist = VLCHG.capture_histogram(source=0)
...
while True:
    ...
    VLCHG.detect_face(frame, block=True)

    # detect the hand
    hand = VLCHG.detect_hand(frame, hist)
    ...
    cv2.imshow("VLCHG", quick_outline)

    # display the unprocessed, segmented hand
    # cv2.imshow("VLCHG", hand.masked)

    # display the binary version of the hand
    # cv2.imshow("VLCHG", hand.binary)
